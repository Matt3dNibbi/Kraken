...
class GraphView(QtGui.QGraphicsView):
    ...
    def rearrangeNodes(self):
        ...
        # def getSrcNodes(node, connections, srcNodes, nodeConnections):
        ...
        #             newSrcNodes.append(srcNode)
        #             srcNodes.append(srcNode)

        #     for eachNode in newSrcNodes:
        #         getSrcNodes(eachNode, connections, srcNodes, nodeConnections)

        # srcNodes = []
        # nodeConnections = {}
        # for node in nodes:
        #     getSrcNodes(node, connections, srcNodes, nodeConnections)

        def getAllNodeConnections():
            keys = {}
            result = {}
            nodes = self.getNodes()
            for k, v in nodes.iteritems():
                ...
                for conn in self.__connections:
                    ...
                    key = "{} - {}".format(srcNode.getName(), dstNode.getName())
                    if keys.has_key(key):
                        continue
                    keys[key] = True

                    if srcNode == v:
                        result[k] += [dstNode.getName()]

            return result

        def getNodeConnections(nodeName):
            keys = {}
            result = []
            ...
            for conn in self.__connections:
                ...
                if scrNode == node:
                    result += [dstNode.getName()]

            return result

        def getMinConnectionPortIndex(sourceNode, targetNode):
            minIndex = 10000
        ...
        # Originally implemented by Helge Mathee
        nodes = self.getNodes()
        nodeNames = self.getNodes().keys()
        nodeConnections = getAllNodeConnections()
        ...
        for n in depth:
            ...
            numPorts = len(nodes[n].getInputPorts()) + len(nodes[n].getOutputPorts())
            if numPorts > maxPortsPerRow[depth[n]]:
                maxPortsPerRow[depth[n]] = numPorts
